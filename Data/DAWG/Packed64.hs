...
-}
type NodeVector = V.Vector Word64
...
pack :: Char -> Bool -> Bool -> Int -> Word64
...
unpack :: Word64 -> NodeVector -> Node
unpack !n !v = Node {
    nodeVector = v,
    childIndex = (n .&. (complement $ zeroBits)) `shiftR` 10,
...
getNodeAt :: NodeVector -> Word64 -> Node
...
reduce :: Trie -> S.State (HM.HashMap [Word64] Int, Int) Word64
